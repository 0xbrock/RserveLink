...
{
    /// <summary>
    /// Class supported Unix crypt method.
    /// </summary>
        ...
        private static byte[,] s_boxes = 
	{
        ...
        {
            byte[] x = new byte[64];
            Array.Copy(data, 0, x, 0, x.Length);
        ...
        private static void rotate(byte[] key)
        {
            ...
            x[27] = key[0];
            x[55] = key[28];
            Array.Copy(x, 0, key, 0, key.Length);
        ...
        private static void f(int i, byte[] key, byte[] a, byte[] x)
        {
            ...
            Array.Copy(key, 0, ikey, 0, ikey.Length);
        ...
        {
            Array.Copy(k, 0, key, 0, key.Length);
        ...
        public static String crypt(String strpw, String strsalt)
        {
            ...
            Array.Copy(etr, 0, new_etr, 0, new_etr.Length);
            EP = new_etr;
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (c > 'Z')
                {
                    c -= (char)(6 + 7 + '.');	// c was a lowercase letter
                }
                else if (c > '9')
                {
                    c -= (char)(7 + '.');		// c was a uppercase letter
                }
                else
                {
                    c -= '.';			// c was a digit, '.' or '/'
